...
#packnames = ('seewave', 'soundgen','tuneR','warbleR')
...
specan3 <- function(X, bp = c(0,22), wl = 2048, threshold = 5, parallel = 1){
  ...
  #if any selections longer than 5 secs stop
  if(any(end - start>5)) stop(paste(length(which(end - start>5)), "selection(s) longer than 5 sec"))  
...
#setwd("C:/Users/abhin/Desktop/GenderRecognition")

# Write out csv dataset.
write.csv(data, file='my_voice.csv', row.names=F)
}
"""
Write= SignatureTranslatedAnonymousPackage(string, "powerpack")
Write.W()
